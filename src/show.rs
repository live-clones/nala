...
pub fn format_dependency(base_dep: &BaseDep, red: bool) -> String {
...
pub fn show_dependency(depends: &[&Dependency], red: bool) -> String {
	...
	for (i, dep) in depends.iter().enumerate() {
		...
		if dep.is_or() {
			for (j, base_dep) in dep.base_deps.iter().enumerate() {
				depends_string += &format_dependency(base_dep, red);
		...
		depends_string += &format_dependency(dep.first(), red);
		depends_string += dependency_footer(total_deps, i);
	}
	depends_string
}

pub fn format_local(pkg: &Package, pacstall_regex: &Regex) -> String {
...
pub fn show_version<'a>(
	...
	let mut version_map: Vec<(&str, String)> = vec![
		(
			"Package",
			config().color.package(&pkg.fullname(true)).into(),
		),
	...
	if let Some(pkg_file) = ver.package_files().next() {
		...
		if let Ok(archive) = pkg_file.archive() {
			if archive == "now" {
				source += &format_local(pkg, pacstall_regex);
			} else {
	...
	for (header, deptype) in dependencies {
		if let Some(depends) = ver.get_depends(&deptype) {
			...
			version_map.push((
				header,
				show_dependency(&deduped_depends, red)
					.trim_end()
					.to_string(),
			));
		}
...
pub fn show() -> Result<()> {
	...
	for pkg in virtual_filter(packages, &cache)? {
		let versions = pkg.versions().collect::<Vec<_>>();
		additional_records += versions.len();

		if config().get_bool("all_versions", false) {
			for version in &versions {
				show_version(&pkg, version, &pacstall_regex, &url_regex);
